...
| `DESIGN` | se requiere una dirección, sistema y especificación o prototipo | `UI_DESIGN_PACKET` |
...
Si el operador declara `SPEC_ONLY`, respetar ese alcance como estatus epistémico
dentro de `FRAME`, `MODEL` o `DESIGN`: producir la especificación solicitada y dejar
la validación pendiente. Si no declara modo, inferir el menor modo que complete la
acción; ante ambigüedad material preguntar o entregar el trabajo independiente, sin
activar `FULL` por defecto. `FULL` se usa sólo cuando el encargo pide el bucle completo.
...
- `no-comparable-directions` — el encargo exige una decisión comparativa y no existen
  alternativas materialmente comparables;
...
Cuando el encargo exige comparar direcciones, deben materializarse con el mismo
fixture, estados y soportes antes de elegir. Una dirección única fundada o una salida
de investigación no obliga a fabricar alternativas.
...
Para TLHD o un binding paciente/episodio equivalente, y sólo cuando exista una
decisión comparativa material, usar estas direcciones candidatas con el mismo contenido
y estados:
...
Cada dirección comparada incluye los soportes y estados pertinentes. No tratarlas como
respuesta universal ni producir tres skins de la misma estructura. Si la evidencia ya
sostiene una dirección, registrar la decisión y omitir divergencia artificial; si el
binding no corresponde, enrutar al método general.
...
- `UI_DESIGN_PACKET`: dirección y alternativas comparables cuando correspondan,
  decisión, sistema, composiciones web/mobile web, estados, spec o prototipo y handoff;
...
4. `G4-concepts` — cuando hay comparación, direcciones bajo el mismo fixture;
...
Aplicar sólo los gates que correspondan al modo y a la promoción solicitada. Un gate
rojo mantiene la afirmación afectada como propuesta o pendiente; no invalida por
rutina una especificación `SPEC_ONLY`. `FRAME` y `DESIGN` sintéticos pueden registrar
G8 pendiente; `BUILD` productivo se detiene antes de implementar semántica sensible
sin G8.
...
7. Cuando se comparan direcciones, usar fixture y estados equivalentes antes de elegir.
...
- El usuario solicita `SPEC_ONLY` con fixture sintético: entregar la especificación
  visual y marcar runtime, accesibilidad y validación clínica como pendientes; no
  exigir prototipo ni recibo clínico para describir forma no promovida.
- La clasificación es `phi` o `unknown`: detener lectura y captura, pedir fixture
  sintético o desidentificado y no repetir el contenido.
- Existe una sola dirección fundada para una microedición: materializarla dentro del
  sistema; no producir tres conceptos por cuota.
- Una alerta clínica requiere semántica o política no autorizada: diseñar estructura y
  estados sintéticos, pero no aprobar criterio, umbral, prioridad ni conducta clínica.
