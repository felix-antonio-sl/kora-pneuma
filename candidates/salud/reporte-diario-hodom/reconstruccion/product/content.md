...
- preselección separada de atenciones adultas observadas en Unidad de Emergencia
  y pacientes hospitalizados en Medicina, Traumatología y Cirugía que ameritan
  evaluación formal para HODOM;
...
3. Codex o Hermes exponen efectivamente las herramientas de terminal, lectura y
   escritura necesarias y el encargo autoriza usarlas. El target declarado no
   prueba por sí solo esa disponibilidad.

El entorno y el encargo efectivos determinan dónde puede procesarse o escribirse
PII/PHI; esta skill no presupone un perfil personal universal. El reporte
identificable se escribe sólo en una superficie clínica privada autorizada.
Chat, Git y artefactos publicables reciben estado y conteos desidentificados,
nunca el contenido clínico identificable.
...
2. Leer `state`, `error_code`, `affected_systems`, `outage_kind` y latencias
   cuando estén presentes. Interpretar por separado `health_status`,
   `beta_ready`, `all_capabilities_ready` y
   `systems.*.data_quality_status` sólo si la respuesta observada los emite;
   una clave ausente no equivale a `false`.
...
`urn:salud:kb:manual-agente-hsc-agent-cli`, cuya revisión vigente documenta
contrato `beta-4` y guía `agent-autonomy-13`. Ese pin no acredita la capacidad
viva: leer `--version`, el `--help` del subcomando y la respuesta observada; no
reconstruir el contrato desde recuerdos.
...
Ejecutar en serie las órdenes de `batch_plan.requests[]` emitidas, respetando
`execution_order`. En un lote de dos o más handles, usar `--stream` sólo si la
ayuda vigente lo admite y no combinarlo con `--budget-bytes`. Si existe un solo
handle listo y por ello no hay `batch_plan`, ejecutar un bundle single sin
`--stream`; para un `--handoff` individual se puede pedir
`--budget-bytes 16384`. No aumentar concurrencia externa ni delegar pacientes en
paralelo.
...
- Unidad de Emergencia como cohorte DAU; presencia en UE no demuestra
  hospitalización ni boarding;
- Medicina;
- Traumatología;
...
2. **Profundización selectiva:** construir lotes únicamente con los handles
   cuya selección tenga una razón factual registrada. Respetar la forma y el
   tamaño admitidos por la ayuda vigente y sintetizar un paciente por vez.
   La selección identifica un registro, no crea su direccionabilidad. Conserva
   por separado ID censal y handle emitido por la fuente. Si sólo consta el ID,
   deja `handle: null` y planifica resolverlo mediante una consulta autorizada;
   no lo incluyas como handle en un lote ni lo conviertas en URI. La misma regla
   rige para un plan escrito, aunque todavía no se ejecute.
...
Cuando estén realizados y disponibles en el destino, componer
`urn:salud:artefacto:hospitalista` para flujo y transición,
`urn:salud:artefacto:asistencial-hospital` para lectura del caso y
`urn:salud:artefacto:hospitalizacion-domiciliaria` para la compuerta HODOM. Su
ausencia se declara; no se simula una invocación ni se convierte el censo en
...
**preselección censal**. Antes de autorizar ingreso faltan, como mínimo:
evaluación clínica, necesidad asistencial compatible con la capacidad HODOM
realmente disponible, condiciones domiciliarias, apoyo requerido,
consentimiento, cobertura operacional y contingencia efectiva.
...
2. Usa sólo handles emitidos por la fuente. `ingreso_id` identifica el episodio
   SGH, pero un ID censal o `cp` no se convierte por sí solo en handle.
...
Las URN de esta sección son relaciones de composición. Sólo una dependencia en
`requires` y una realización disponible permiten exigir su procedimiento; la
arista por sí sola no prueba invocación runtime. No copiar sus procedimientos al
reporte.
