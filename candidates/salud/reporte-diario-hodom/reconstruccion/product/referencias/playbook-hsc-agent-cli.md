...
La referencia vigente documenta contrato `beta-4`, binario fuente
`v3.2.0-31-g89f273a` y guía `agent-autonomy-13`. Esa revisión acredita el
contrato consultado, no la disponibilidad viva de una instalación o fuente. Al
ejecutar, leer `--version`, `health`, la ayuda del subcomando y los campos
realmente emitidos. La ejecución fallida del 2026-07-28 conserva un aprendizaje
del consumidor: no materializar servicios candidatos completos y mantener un
...
```text
hsc-agent-cli --version
hsc-agent-cli health
```

Registrar `fetched_at`, `data.health_status`, `data.systems`,
`data.beta_ready`, `data.all_capabilities_ready`,
`data.degraded_capabilities` y `data.partially_probed_systems` cuando la
respuesta observada los emita. Una clave ausente no equivale a `false` ni
acredita capacidad.
...
el mismo argv. Cuando sea un lote de dos o más y la ayuda vigente lo admita,
agregar:
...
No alterar handles ni modo. No ejecutar dos requests del plan en paralelo. Un
presupuesto de salida no se combina con lote ni stream; revisar `compaction`,
`_compaction`, `truncated_keys` y `truncated_by_limit` donde aparezcan.
...
Para dos o más handles SGH seleccionados, construir un lote sólo con los casos
justificados y respetar la ayuda vigente:
...
Para dos o más handles DAU seleccionados:
...
Con un solo handle, omitir `--stream`. El tamaño admitido, las opciones y el
costo se toman de la ayuda y la respuesta de esa instalación; este playbook no
fija una capacidad no observada.
...
- Usar el costo emitido por `batch_plan` como estimación primaria y declarar
  cuando no esté disponible; no convertir una medición histórica en SLA.
