...
Antes de modelar, comprobar con los hechos disponibles que el sistema tiene
**funcion transformadora identificable**. Si no está explícita, puede ayudar
esta pregunta:
...
La clasificacion no es decorativa: decide qué componentes aplican. Si no puede
cerrarse todavía, marcarla como propuesta y evitar las afirmaciones que dependan
de esa clasificación; preguntar sólo si el entregable exige decidirlas.
...
Una transformación principal por proceso. Varios verbos pueden describir pasos,
aspectos coordinados o funciones distintas; decidir entre proceso único,
in-zooming o sistemas distintos por sus transformees, resultados y frontera,
no por el número de verbos.
...
Usar los agentes humanos u organizacionales aportados. Si ese rol importa y no
está definido, dejarlo abierto o preguntar; si se sabe que no existe, registrar
`sin agentes humanos`. No inventar un placeholder.
...
- [ ] cada hecho tiene OPL-ES equivalente según la comprobación ejecutada.
- [ ] la interpretación de dominio se identifica como aportada, propuesta o
      validada por la autoridad correspondiente; no exigir una ratificación
      humana nueva cuando el encargo ya la aporta o sólo pide un borrador.
...
- SD cuya densidad impide leer la función o seguir los enlaces: evaluar
  in-zooming según audiencia, propósito y complejidad. Los rangos de claridad
  del método son heurísticas, no una cuota que obligue a refinar.
