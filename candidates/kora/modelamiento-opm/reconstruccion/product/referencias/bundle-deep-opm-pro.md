...
Documento JSON canónico que la skill `modelamiento-opm` emite cuando el destino
es **edición / refinamiento / revisión** en el modelador `deep-opm-pro`. Resolver
la ruta del checkout desde el encargo o el workspace efectivo.
...
`<deep-opm-pro-root>/app/`; snippet en la ruta `operacion-forja`). El
...
4. Adjuntar el bundle al entregable y dar al usuario el camino de import:
   `cd <deep-opm-pro-root>/app && bun run dev` → **Abrir / importar
