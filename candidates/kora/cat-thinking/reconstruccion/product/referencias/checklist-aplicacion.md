...
Usar despues de aplicar un patron del corpus, antes de entregar al agente invocador. Aplica sólo los bloques cuya estructura se afirme; una heurística no debe construir un topos para pasar el checklist. Marca con tres niveles: **CRITICO** (debe pasar), **MEDIO** (debe pasar o declararse explicitamente), **BAJO** (recomendable).
...
- [ ] **CRITICO** todo agente llamado coálgebra identifica su funtor y
  estructura. Si se usa el modelo de entrada/salida con efectos, exhibe `I`,
  `O`, `U`, la mónada `M` y `step : U × I -> M(O × U)`; no es la única forma
  posible de coálgebra.
