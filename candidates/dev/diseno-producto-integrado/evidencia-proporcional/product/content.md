...
- `DESIGN_PACKET`: encuadre, mapa de fuerzas, dirección y alternativas cuando
  correspondan, decisión y rechazos, resultado suficiente —investigación,
  especificación, sistema o prototipo—, evidencia, riesgos, deuda y siguiente paso.
...
- `reality-check-failed`: la propuesta falla uso real y no converge después de
  las iteraciones justificadas por el encargo;
...
Precondición: existe un flujo o artefacto para `AUDIT`, o un encargo
`SPEC_ONLY` para especificar UX; ese modo no acredita uso observado. Postcondición: cada
...
Activar esta etapa cuando la decisión requiere comparar conceptos. Generar las
direcciones necesarias para representar alternativas materialmente distintas; las
siguientes tres son arquetipos útiles, no una cuota:
...
No producir tres skins de la misma estructura. Si el encargo puede resolverse con una
dirección fundada o es investigación sin evidencia para diseñar, documentar esa ruta
y omitir divergencia artificial.
...
Materializar el **menor bucle completo** cuando la decisión necesita aprender de una
forma o conducta observable, con contenido realista:
...
Usar el adaptador `I_design -> O_design` cuando se requiera materialización visual.
La primera versión es instrumento de aprendizaje. Si el encargo pide investigación,
dirección o `SPEC_ONLY`, entregar el protocolo o la especificación sin fabricar un
prototipo.
...
1. Conservar fuentes y pruebas; usar `EVIDENCE_LEDGER` numerado si ayuda a revisar. La
   evidencia de cada afirmación debe ser identificable y comprobable: una entrada
   del ledger cuando existe, o la cita directa (insumo, ruta o comando con
   resultado) en la sección de evidencia en caso contrario. La numeración `[E#]`
   es obligatoria cuando hay más de una afirmación `verificado` o la revisión
   necesita referencias cruzadas; con una sola afirmación basta la cita directa,
   sin numerar. Admite
   entradas con procedencia, fuentes leídas y comprobaciones realmente ejecutadas,
   incluso sobre artefactos propios. Una declaración no equivale a corroboración externa.
2. Cada afirmación `verificado` debe citar su evidencia definida en esta ejecución:
   `[E#]` más insumo, ruta o comando con resultado, o cita directa cuando el caso
   admite la forma simple. Esa entrada o cita es su fuente observable de esta
   ejecución. Un `[E#]` citado sin entrada correspondiente no acredita nada.
3. La mera generación de una especificación, código, token o componente no
   acredita su corrección. Una comprobación debe identificar criterio, método
   y resultado, con un alcance que no dependa de aceptar la conclusión como premisa.
...
6. Sin artefacto ejecutable, tratar el resultado como especificación: WCAG,
   ...
   `pendiente`.
7. Si faltan decisiones de diseño, proponerlas con fundamento o usar placeholders
   según el encargo; no convertirlas en infraestructura o política existente.
...
- canon de gusto se rotula como lente; normas, por versión y aplicabilidad;
  ninguno prueba cumplimiento del producto;
- la solución diseñada se rotula `propuesto` y sus pruebas `pendiente`;
- se pueden proponer números, tokens, componentes y opciones con fundamento;
  no se inventan datos observados, capacidades existentes ni compromisos ajenos;
- no se afirma que la propia especificación cumple, funciona o está probada.

Si un fallo crítico persiste después de las iteraciones justificadas por riesgo,
evidencia y costo, devolver `reality-check-failed`.
...
Repetir `prototipar -> probar -> integrar` mientras una nueva iteración pueda resolver
un fallo material dentro del alcance.
...
5. Cuando se prototipa, representa un bucle completo y sus fallos.
...
14. La generación no acredita corrección ni capacidades instaladas. Una
    comprobación real sobre el artefacto generado acredita sólo su alcance.
...
## Mapa de capacidades y pérdidas corregidas

| Capacidad | Pérdida evitada | Regla situada |
|---|---|---|
| Dirección | Menú de alternativas sin decisión | Divergir sólo ante una comparación material y elegir cuando exista evidencia |
| Investigación | Entrevistas o prototipos inventados | Puede terminar en protocolo, hipótesis y preguntas pendientes |
| Forma visual | Prosa genérica sin artefacto | Activar `design` cuando la decisión requiere materialización |
| Auditoría UX | Gusto presentado como usabilidad | Activar `ux-design` sólo sobre criterios y evidencia observables |
| Prueba de realidad | Especificación presentada como funcionamiento | Conservar `SPEC_ONLY`, niveles de evidencia y pruebas posteriores |
| Continuidad | Handoff que pierde intención | Entregar decisión, sistema, evidencia, deuda y siguiente acción |

## Casos discriminantes

- Investigación sin participantes: entregar protocolo e hipótesis, sin tres
  direcciones ni prototipo ficticio.
- Dos conceptos comparables: usar el mismo fixture y estados, elegir y registrar las
  pérdidas de la decisión.
- Microajuste visual con dirección cerrada: activar `design` directamente y omitir el
  recorrido amplio.
- Especificación sin runtime: respetar `SPEC_ONLY`; accesibilidad y conducta quedan
  propuestas o pendientes.
