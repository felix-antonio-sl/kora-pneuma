...
```mermaid
flowchart LR
  U[Félix en Telegram] --> T[Adaptador de conversación y controles]
  T --> S[Servicio de dominio y control]
  S --> D[(SQLite y originales privados)]
  S --> H[Adaptador Hermes: un intento nativo activo]
  H --> S
