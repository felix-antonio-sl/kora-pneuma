...
Edición `1.0.22`, sincronizada el 2026-09-05 con la candidata local `beta-4`
del repositorio `hsc-agent-cli`, todavía sin tag ni publicación. Conserva el
corte post-`v3.2.0` documentado por la edición anterior y actualiza el consumo
del resultado operativo, las salidas acotadas, los errores de tamaño y la salud
por etapas. El flujo estándar es
autoritativo en
`<comando> --help`; este manual conserva inventario exhaustivo, caveats de
fuente y doctrina excepcional.

Procedencia de esta edición: contrato `beta-4` de la candidata local y su
decisión técnica vigente en el repositorio `hsc-agent-cli`, observados el
2026-09-05. No se atribuye tag, release ni commit a una candidata todavía no
publicada; la historia de procedencia de las ediciones anteriores permanece en
el campo `fuente`.

La edición `1.0.21` documenta el sondeo de transporte ESB de HCC en `health`
(handshake TLS por puerto, `esb_transport_ok*` con HCC como capacidad no
requerida para `beta_ready`) y el flag `--with-hcc` de los bundles de episodio
(resúmenes Rayen APS anclados al RUT del episodio, fail-closed por ancla y
fallo aislado `upstream_unavailable` cuando el ESB de datos cae). Sobre
hsc-agent-cli@121a87b (post v3.2.0).

La edición `1.0.20` separa la salud del núcleo requerido, la completitud de
capacidades y la calidad de datos de Drive. Una fila material no parseable
mantiene utilizables las coincidencias con identidad válida, pero impide leer
una búsqueda negativa como ausencia.

La edición `1.0.19` actualiza únicamente la entrada documental: contrato
operativo en `AGENTS.md`, uso en la ayuda ejecutable y estado observado en
vivo. No modifica el contrato del CLI.

La edición `1.0.18` conserva el contrato factual de `v3.1.1` y adopta la guía
post-tag `agent-autonomy-5`: un censo singleton omite `batch_plan` y se resuelve
por `entry.handle`/`best_current_context`; en stream se inspeccionan
`envelope.state`/`envelope.error_code` antes de un `envelope.summary` opcional.
...
**Jerarquía de verdad** (si algo aquí contradice la realidad): mandan el
binario, su ayuda y sus tests; `~/projects/hsc-agent-cli/AGENTS.md` fija el
contrato operativo del repositorio y este manual guía el consumo. Detalle
técnico de endpoints upstream: `~/projects/hsc-agent-cli/docs/reference/`.
...
| `bundle <handle> --minimal\|--compact\|--handoff\|--longitudinal` | caro | Componer varios handles de un encuentro en un paquete; `--with-hcc` agrega resúmenes HCC (Rayen APS) anclados al RUT del episodio |
...
| `health` | barato | Verificar conectividad upstream + versión del binario; reporta por sistema `ok`, `latency_ms` y `checked_at` de esa invocación; HCC incluye sondeo de transporte ESB por puerto (`systems.hcc.transport.ports[]`) |
...
primer arg) responde un envelope beta-4 (`kind:"help"`, `error_code:null`,
...
versión `agent-autonomy-6`, con flujo completo, selección de primitive,
...
`bundles[]`. La guía vigente también expone el `handle` directo de cada entry
DAU, `error_detail.supported_modes` ante modo bundle incompatible y exige que
una `--sala <id>` numérica coincida con la enumeración SGH actual. La ayuda declara
...
|---|---|
| 0 | ok |
| 2 | `usage_error` (input mal formado) / `budget_too_small` |
| 3 | `patient_not_found` / `identity_mismatch` |
| 4 | `upstream_unavailable` / `not_implemented_yet` / `response_too_large` |
| 5 | `internal_error` (bug del CLI) |

En bundles, el envelope se emite antes de terminar el proceso y
`operation_status` gobierna el resultado: `complete` y `partial` conservan
`ok:true` y exit 0; `failed` emite `ok:false`, `state:error`, un
`error_code` agregado estable y exit no cero. Una ausencia factual no cuenta
como falla. La regla también cubre lotes y la línea terminal de `--stream`.
...
| `contract_version` | versión del contrato (`beta-4`); detecta cambios de shape |
...
Todo bundle agrega `operation_status=complete|partial|failed`. En `--compact`
y `--handoff`, `items` es un índice recuperable con handle, estado, procedencia,
`command_args` y `cache_write_failed` cuando corresponde; el payload íntegro no queda inline y
`items_payload_omitted:true` lo declara. La proyección mecánica permanece en
`data` para `--compact` y en `handoff_view` para `--handoff`. Recupera detalle
ejecutando los `command_args` o consultando el handle; no leas el índice como
contenido clínico vacío.
...
| `response_too_large` | la respuesta HTTP, CSV, JSON o PDF excedió la cota segura de adquisición | no uses contenido parcial ni lo conviertas en ausencia; acota la consulta si existe un knob público o reporta el límite. No es caída upstream ni bug interno |
| `budget_too_small` | el envelope obligatorio de un bundle single `--handoff` no cabe en `--budget-bytes`; exit 2 | aumenta el presupuesto o retira el flag; no interpretes la ausencia de payload como ausencia factual |
...
la superficie. `health_status=healthy` significa que el núcleo requerido está
disponible; no certifica completitud de capacidades ni calidad total de datos.
Por eso `all_capabilities_ready=false` puede coexistir con `beta_ready=true`,
`ok:true`, `state:present` y exit 0. Una caída requerida emite `ok:false`,
`state:error`, `health_status:unavailable` y exit 4.

En Drive, acceso y esquemas válidos mantienen `availability=available`. Si una
fila material no puede parsearse, `data_quality_status=partial` y
`probe_result=partial`: `positive_lookup_usable=true` conserva coincidencias
con identidad válida, mientras `negative_lookup_conclusive=false` prohíbe
convertir el universo incompleto en ausencia. Acceso, pestaña o esquema
inválido sí degradan Drive y dejan `data_quality_status=not_assessed`.

La salud se declara por etapas y no promueve evidencia local a disponibilidad
remota. SGH prueba el cambio de contexto hospitalario y sólo renueva sesión si
ese probe observa el formulario de login. DAU y LAB no tienen un endpoint
remoto no-PHI que pruebe inequívocamente una cookie cargada: declaran
`probe_result=partial`, `availability=configurational_only` y mantienen
`beta_ready=false`. HCC separa transporte TLS, autenticación de aplicación y
prueba con paciente. Un peer TLS verificado sólo acredita
`probe_result=partial` y `availability=transport_only`; mientras `auth_probe` y
`real_patient_probe` estén `not_run`, `all_capabilities_ready=false`. HCC sigue
siendo complementario, por lo que esa parcialidad no vuelve por sí sola
`beta_ready=false` cuando el núcleo requerido está listo.
...
bundle urgencia:dau:<atencion_id> --minimal --with-hcc  # + resúmenes HCC (Rayen APS) del RUT del episodio
bundle paciente:<rut> --longitudinal           # historia contextual mecánica
```

**`--with-hcc` enriquece el episodio con APS sin cambiar de comando**: compone
`hcc:primaria:<rut>/resumen` + `hcc:secundaria:<rut>/resumen` anclados al RUT
que el propio bundle ya adquirió (raíz DAU en urgencia, `estado-actual` SGH en
hospitalización). Opt-in: sin el flag la salida es byte-identica. Fail-closed:
si el ancla no expone RUT, cero items HCC y la omisión se declara en
`warnings[]`. Si el ESB de datos HCC está caído (`health` →
`systems.hcc.esb_transport_ok:false`), los items HCC caen aislados con
`upstream_unavailable` sin degradar el resto del bundle — reintentar más tarde
o usar `paciente:<rut> --longitudinal`. Redundante con `--longitudinal` (que ya
compone HCC): el CLI lo rechaza con `usage_error` en vez de ignorarlo.
...
bundle hospitalizacion:sgh:<ingreso_id> --handoff --budget-bytes 16384
                                       # límite duro del envelope emitido
...
`--budget-bytes` sólo se admite en un bundle **single** con `--handoff`. Se
rechaza con `usage_error` al combinarlo con varios handles o con `--stream`;
el presupuesto no se reparte ni se aplica por componente. La cota cuenta los
bytes realmente emitidos, incluido el salto de línea. Si el envelope obligatorio
no cabe, el CLI responde `budget_too_small`, `ok:false` y exit 2; no entrega un
éxito sobredimensionado.
...
`batch_plan`. Si un listado tiene un solo handle listo, `batch_plan` se omite
por contrato: sigue `entry.handle` o `best_current_context` y ejecuta un bundle
single; la ausencia de plan no significa ausencia de paciente o episodio.
...
línea NDJSON por separado. En cada componente inspecciona primero
`envelope.state` y `envelope.error_code`; consulta `envelope.summary` solo si
existe. Un sub-envelope fallido puede no producir `summary`.
...
enumerable**: el listado se entrega con su alcance incompleto. Si no existe un
match positivo, el envelope usa `state=error`, `error_code=upstream_unavailable`
y conserva los flags; `no_en_censo`/`ausente` queda reservado al barrido y
enumeración completos. Lo que NO puedes hacer es
...
limitación aparece como `source_issues[].issue_kind=source_scope_incomplete` y
el item conserva `state=error`; nunca como ausencia confirmada.
...
- Una fila no encontrada solo es ausencia de esa fuente si el probe declara
  `negative_lookup_conclusive=true`. Con esquema válido y una fila material no
  parseable, Drive sigue `available` con `data_quality_status=partial` y
  `positive_lookup_usable=true`, pero las búsquedas negativas no son
  concluyentes. Planilla caída, pestaña ausente o esquema inválido producen
  `data_quality_status=not_assessed` y fallo del probe, no una ausencia ni una
  brecha clínica fabricada.
...
| `operation_status` | todo bundle, lote y summary terminal de stream | `complete`: ningún componente falló; `partial`: coexisten componentes fallidos y no fallidos; `failed`: todos fallaron. La ausencia factual no cuenta como falla |
| `items_payload_omitted` / `items[].command_args` / `items[].cache_write_failed` | bundles `--compact` y `--handoff` | `items` es un índice recuperable y no contiene el payload íntegro; usa el handle o argv estructurado para profundizar. `cache_write_failed`, si existe, preserva el hecho mecánico de que falló la escritura de caché y no altera por sí solo el contenido adquirido |
| `source_issues[]` | `summary.source_issues[]` en bundle normal; `data.source_issues[]` en `--compact`; `handoff_view.source_issues[]` en `--handoff` | hecho por fuente: `source_returned_no_content`, `acquisition_error`, `identity_mismatch`, `source_scope_incomplete` o `unknown`; `alternative_handles[]` no expresa equivalencia, severidad ni orden. En handoff, `summary` no conserva `source_issues[]` |
| `bundle_integrity` | `summary.bundle_integrity` en bundle normal; `data.bundle_integrity` en `--compact`; `handoff_view.bundle_integrity` y también `summary.bundle_integrity` en `--handoff` | identidad y adquisición solamente. Lee `identity.status`, `acquisition.status` y `does_not_assess_clinical_safety:true`; nunca lo conviertas en autorización terapéutica o suficiencia clínica |
...
| `compaction` / `_compaction` / `truncated_keys[]` | salidas `--compact`/`--budget-bytes` | hubo pérdida mecánica declarada; `truncated_keys[]` lista las claves exactas truncadas. `--budget-bytes` es límite duro sólo para bundle single `--handoff`; si el envelope obligatorio no cabe devuelve `budget_too_small` |
...
| `health_status` / `beta_ready` / `all_capabilities_ready` | `health.data` | salud del núcleo, readiness requerida y completitud total son ejes distintos; `healthy` no certifica completitud |
| `data_quality_status` / `positive_lookup_usable` / `negative_lookup_conclusive` / `remediation_code` | `health.systems.drive` | calidad de las filas, utilidad de coincidencias positivas, conclusividad de búsquedas negativas y reparación de fuente correspondiente |
...
- Contrato operativo del repositorio: `~/projects/hsc-agent-cli/AGENTS.md`;
  `CLAUDE.md` lo importa sin duplicarlo.
- Uso público: ayuda ejecutable del binario. Comportamiento realizado: código y
  pruebas del repositorio.
- Referencia técnica de endpoints upstream: `~/projects/hsc-agent-cli/docs/reference/`.
- Estado operacional: Git, binario y fuentes autorizadas observados en vivo;
  `HANDOFF.md` solo si existe y se retoma trabajo inconcluso.
