...
Edición `1.0.19`, sincronizada el 2026-07-18 con la fuente viva de
`hsc-agent-cli` (contrato `beta-3`; último release `v3.1.1`, build post-tag
limpio `de1e0b7`) y su guía inline `agent-autonomy-5`. El flujo estándar es
autoritativo en
...
versión `agent-autonomy-5`, con flujo completo, selección de primitive,
...
figura en `partially_probed_systems` porque su prueba es configuracional. Para
DAU, SGH y LIS, `health` fuerza un login nuevo: una cookie cargada desde disco
no se toma como autenticación viva. HCC conserva `transport_probe` y
