...
Lee [el protocolo](references/protocolo.md) antes de la primera ejecución. El helper requiere Python 3.10 o posterior en Linux. Guarda fuentes congeladas y evidencias en una carpeta autorizada para esos datos, fuera de las fuentes, Git y bundles. `state.json` conserva el texto completo de entrada: comprueba ubicación y permisos de esa carpeta, incluidos los preexistentes, antes de incorporar material protegido. El helper no decide su autorización. Ejecuta el script por ruta absoluta o desde la skill:
...
El helper comprueba una compresión de fuentes identificadas; no adjudica
contradicciones ni incorpora una investigación nueva por iniciativa propia. Si
el encargo exige recomponer conocimiento con nuevas fuentes, identifícalas y
conserva sus atribuciones antes de compararlas. Una fuente extranjera conserva
su jurisdicción y no adquiere aplicabilidad local por la transformación. Una
parte faltante o un conflicto interno permanece junto a la afirmación afectada.
Si una explicación necesaria aumenta el costo, conserva la candidata fiel y
explica el límite del helper: no elimines significado para obtener un recibo.
La medida de compresión no es requisito de aprobación editorial de KORA.
