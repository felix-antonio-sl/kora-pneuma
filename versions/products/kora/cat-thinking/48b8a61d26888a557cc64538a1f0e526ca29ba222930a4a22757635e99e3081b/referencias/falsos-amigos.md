...
| Lo que parece | Lo que es |
|---------------|-----------|
| "puedo entender un objeto por su API" | un objeto `A` queda determinado salvo isomorfismo por su funtor representable `Hom(A, -)` considerado salvo isomorfismo natural. |
...
| "log distribuido" | ni siquiera es presheaf por su nombre: primero declarar sitio, objetos y restricciones functoriales; después comprobar pegado para afirmar sheaf. |
...
| misma firma PMI × LFS en una fuente histórica | misma celda de esa clasificación heredada, retirada de la maquinaria vigente; no mismo agente, interfaz, coálgebra ni conducta. |
