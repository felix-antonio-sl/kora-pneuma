...
- `urn:kora:kb:cat-kora-kernel` — fuente, identidad, dependencias y realización;
- `urn:kora:kb:cat-kora-semantica-operacional` — efectos, propiedad, publicación y recuperación;
...
| KORA | `cat-kora-kernel` | modelo vigente de fuente, identidad, dependencias y realización |
| KORA-op | `cat-kora-semantica-operacional` | efectos y límites de publicación, instalación y recuperación |
...
2. **¿Es un problema operacional con respuesta directa?** Si sí, responder directamente sin desplegar esta lente.
3. **¿Admite lectura categorial sustantiva?** Si no, conservar la representación operacional que sí ayuda y declarar el límite cuando sea relevante.
...
Si ninguno resuelve, declarar la laguna y consultar una fuente primaria pertinente
cuando pueda resolverla. Continuar lo independiente; no inventar una garantía.
...
8. **No fuerces la lente**. Si no aporta, responde operacionalmente dentro de la
   capacidad disponible; deriva sólo la parte que requiera otra especialidad.
