# Operacion profunda de modelamiento OPM/Forja

> Actualizacion 2.1.0 (2026-07-27): alinea la skill con el ciclo reversible de
> opforja. Distingue Taller/Modelos/Bibliotecas/Archivo, separa
> Apunte⇄Modelo de Boceto⇄OPD integrado, incorpora Reabrir y Devolver, y delega
> el gate de Bocetos a R-CAN-BOCETO-1..4. Conserva los identificadores internos
> `adoptarOpd` y `origen:"adopcion"` por compatibilidad.
>
> Corrección 2.0.2 (2026-07-18): sincroniza el checklist categorial con la
> regla viva: igualdad de firma implica equivalencia observacional relativa,
> no identidad, bisimulación ni sustituibilidad total.
...
|------|-----|-----------------|
| Validez Forja | `urn:fxsl:kb:reglas-opm-estrictas-es` (v1.5.0) | SSOT primaria: validez operativa, severidad, defaults, extensiones declaradas, anti-patrones AP-01 a AP-30, checklist OPD<->OPL y Anexo C. R-CAN-BOCETO-1..4 gobierna el gate de Bocetos por regimen. |
| Realizacion OPD | `urn:fxsl:kb:spec-forja-opd-es` (v1.4.0) | SSOT visual de opforja: geometria, canvas, render, edicion visual, export y bisimetria visual. §10.4 R-OPD-REF-20 realiza Bocetos e integracion reversible con la voz Integrar/Devolver. |
| Realizacion OPL | `urn:fxsl:kb:spec-forja-opl-es` (v1.4.1) | SSOT textual de opforja: vocabulario cerrado, plantillas, parseo, edicion textual, roundtrip y GAPs. R-ENT-2-APUNTE exige que los placeholders emitan OPL en toda superficie del Apunte. |
| Metodo Forja | `urn:fxsl:kb:metodologia-forja-opm-es` (v1.7.0) | SSOT primaria del metodo: A0-A8, heuristicas, lecciones Forja, bundle y disciplina humano-agente. A1.5 separa los ciclos Apunte⇄Modelo y Boceto⇄OPD integrado. |
...
Las versiones adjuntas son las vivas al momento de esta emision (v2.1.0, 2026-07-27);
...
2. **No ocultar barro como hecho.** Una ambigüedad se marca y se localiza. Puede
   convivir con un Apunte, una propuesta o una sección independiente; sólo
   bloquea la afirmación, serialización o efecto que depende de resolverla.
...
4. **Distinguir decisión vs. incertidumbre.** Una decisión deliberada se
   conserva como tal. Una aproximación vaga se registra como hipótesis o
   abierto; se pregunta sólo si cambia una decisión material del entregable.
5. **Aclaración focal.** Cuando haga falta, formular una pregunta dirigida con
   el hecho faltante, la regla aplicable y las alternativas legales. No repetir
   hechos ya aportados ni convertir el wizard en interrogatorio.
6. **Rigor proporcional.** Corregir las infracciones formales y preservar los
   vacíos como vacíos. Continuar todo el trabajo que no dependa de ellos.

### Catálogo de barro (ambigüedades y anti-patrones)

Cuando alguno aparece, corregirlo, declararlo o entrar a `aclarar` si impide la
salida solicitada:
...
sobre cualquier estado productivo. **Apunte** y **Modelo** son dos regimenes del
mismo documento OPM: comparten ID, hechos y almacenamiento; cambia cuanto cierre
es exigible. El protocolo de persistencia conserva `especie:
apunte|modelo|biblioteca` como discriminante de compatibilidad, pero no crea dos
materiales hermanos ni una etapa intermedia. Un Apunte **relaja el RIGOR, no la
SEMANTICA**: deja pensar en OPM legitimo sin mentir sobre su cierre.
...
- **El flag manda.** El regimen permisivo se activa **solo** cuando el modelo de la
  ...
  **Revisar para graduar → Graduar Apunte a Modelo**. El toggle que modifica
  `esApunte` y los nombres de componentes React son mecanismos internos, no
  gestos alternativos expuestos. Sin flag, la Postura Dialectica corre completa.
- **Todo nace Apunte en Taller.** La puerta «Nuevo» crea un Apunte instantaneo
  (auto-nombre `Apunte AAAA-MM-DD`, autosave desde el primer trazo). La revision
  de graduacion separa integridad, Bocetos, cierre formal y validacion humana.
  La integridad bloquea; si solo quedan Bocetos o deuda formal, el operador puede
  **Seguir en Taller y corregir** o **Graduar con pendientes**. La skill propone,
  nunca gradua por su cuenta.
- **Cuatro espacios, dos ejes.** El gestor **Trabajo de modelado** separa
  **Taller** (Apuntes), **Modelos** (Modelos de Trabajo), **Bibliotecas**
  (Modelos con rol de reuso) y **Archivo** (retencion). Las carpetas organizan
  Taller y Modelos; Biblioteca no es mayor madurez y Archivo no es invalidez.
...
**Revisar para graduar → Graduar Apunte a Modelo**, vuelve a regir R-ENT-2; la
autoria headless permanece en regimen riguroso.
...
- **Acompanar Bocetos sin exigir SD.** Cuando el operador explora sin
  comprometer un SD, la skill NO fuerza SD-primero. Plasma **Bocetos** —OPDs no
  raiz todavia fuera del arbol— como hechos locales. La validez de metodo queda
  en observacion; la integridad nunca se relaja y sus hechos SI emiten OPL.
- **No confundir los ciclos.** Graduar a Modelo cambia el regimen del documento;
  no integra Bocetos. Integrar como descomposicion o despliegue cambia la
  pertenencia de un componente; no gradua el documento. Un Modelo con Bocetos
  queda **Con pendientes** y su export canonico bloquea por
  R-CAN-BOCETO-1..4, aunque la edicion siga disponible.
- **Integracion via mesa, nunca a mano.** La banda visible **Bocetos** ofrece
  **Integrar como…**. El gesto fija padre y slot mediante el mismo constructor
  que el refinamiento top-down; conserva el contenido autorado del Boceto y
  materializa solo la apariencia derivada necesaria. La skill nunca fabrica el
  vinculo editando JSON. `adoptarOpd` y `origen:"adopcion"` son nombres internos
  de compatibilidad, no voz para el operador.
- **Inversa preservante.** **Devolver a Bocetos** libera el vinculo y conserva
  ID, hechos, geometria, pregunta guia y descendientes. **Eliminar
  refinamiento** destruye el subarbol y es una operacion distinta; ante duda, la
  skill explica el efecto antes de pedir confirmacion.
- **Cuando proponer cada gesto.** Proponer Integrar cuando exista una cosa
  destino y una pregunta de refinamiento claras. Proponer Graduar a Modelo
  cuando el operador quiera volver exigible el cierre, exista o no deuda formal.
  Si hay Bocetos o cierre pendiente pero la integridad esta sana, presentar
  **Seguir en Taller y corregir** y **Graduar con pendientes** sin decidir por
  el operador.
- **Bosquejo por el puente directo.** Un bosquejo que la skill crea en la mesa via
  ...
  bosquejo). `--especie` es contrato de transporte; no cambia la distincion
  conceptual entre regimen documental y estado del componente.

### Ciclo documental: Graduar a Modelo y Reabrir en Taller

Graduar no es un pipeline ni una certificacion. La revision muestra nombre,
destino y cuatro planos: integridad, integracion de Bocetos, cierre formal y
validacion humana. La integridad bloquea. Con integridad sana, **Graduar con
pendientes** es una decision explicita: `esApunte` pasa a ausente y los
diagnosticos de Modelo se reenganchan, sin reparar hechos ni integrar
componentes. Las observaciones son un checklist recomputado, no un estado
persistido.

**Reabrir en Taller** es la inversa de regimen para un Modelo de Trabajo:
conserva `model.id`, hechos, carpeta y registro; confirma una version de
reapertura y vuelve a Apunte. No elimina, no duplica y no borra por si sola una
validacion humana externa. Graduar de nuevo opera sobre el mismo documento. No
existe `promovidoDesdeApunte` ni una casta intermedia.
...
Convención de entrega: producir el menor artefacto que el encargo necesita. Un
modelo conceptual puede cerrar en texto. Emitir OPL-ES estricto, bundle, reporte
o render sólo cuando el destino o la comprobación los requieran.

**Corpus Forja por necesidad.** Para validar, generar o serializar una salida
formal, consultar la sección propietaria necesaria del corpus OPM/Forja SSOT ES.
Resolver `urn:fxsl:kb:reglas-opm-estrictas-es` para validez; las modalidades
`urn:fxsl:kb:spec-forja-opd-es` y `urn:fxsl:kb:spec-forja-opl-es` cuando el
entregable sea visual, textual estricto o roundtrip; y
`urn:fxsl:kb:metodologia-forja-opm-es` para una decisión de método. Usar
`urn:fxsl:kb:opm-categorial-es` sólo si hace falta explicar una ley bajo la
superficie. La precedencia operativa es:
...
**Claridad al salir de triaje**: localizar el barro según su impacto. Corregir
una infracción formal, marcar una propuesta o seguir con las partes
independientes. Derivar a `aclarar` sólo si una decisión faltante impide la
salida solicitada.

### `aclarar`: resolver una decisión material

Estado dialéctico acotado al asunto pendiente. No bloquea trabajo independiente.
...
3. **Emitir una pregunta** con la plantilla sólo si la respuesta es necesaria.
4. **Continuar lo independiente** y mantener pendiente la afirmación o efecto
...
6. **Repetir sólo si otra decisión material aparece**; no recorrer un
   cuestionario universal.

Salida: el estado de origen, con el barro o resuelto o convertido en supuesto declarado.

Anti-patrón de la skill: usar preguntas para posponer una salida que los hechos
ya permiten preparar.
...
**Frontera P3 (2026-06-05).** El léxico abierto de dominio vive en la
skill, no en el compilador. La skill puede proponer mapeos y estandarizaciones;
la autoridad de dominio confirma sólo los mapeos cuya verdad se quiera afirmar.
El compilador determinista sólo debe verificar OPL-ES
...
7. **Devolver un ledger de normalizacion.** Para cada cambio, reportar:
   ...
   (`confirmado`, `propuesto`, `pendiente`, `rechazado`) y deuda. El operador debe poder ver
...
- proto-modelo reescrito en OPL-ES estricto con cada mapeo abierto marcado como
  propuesto, pendiente o confirmado según su evidencia;
...
Aplicar el wizard Forja de System Diagram (ver
`referencias/wizard-sd.md`) usando primero los hechos ya disponibles. La skill
no inventa lo que falta y pregunta sólo por una decisión que impida el
entregable:
...
1. **Propósito / outcome** — expresarlo como transformación. Varios verbos
   pueden pertenecer a subprocesos, aspectos coordinados o funciones distintas;
   contrastar transformees, resultados y frontera antes de separar sistemas.
...
5. **Transformees y benefit-providing object** — identificar qué cosa cambia,
   se consume, se crea o se destruye. Se pueden proponer candidatos rotulados;
   no atribuirlos al operador ni convertirlos en hechos sin evidencia. Si hay
   múltiples transformees, distinguir el objeto que provee la función principal.
...
11. **Links procedurales** — proponer el tipo de link más probable
   (consumo/resultado/efecto/agente/instrumento/condición/evento) citando la
   firma legal; marcar como propuesta cualquier semántica de dominio no aportada.
12. **Bimodalidad y cierre** — emitir OPD estructurado + OPL-ES cuando el
   entregable los exija y comprobar equivalencia mecánica. La aceptación de la
   interpretación de dominio es una validación distinta y se solicita sólo si
   se necesita afirmar como aceptada.
13. **Decisión de refinar** — refinar cuando el encargo o la legibilidad lo
   requieran. Si hay alternativas con efectos distintos, presentar la decisión
   mínima pendiente.

Regla de cierre del estado: el SD declara qué componentes están sustentados,
cuáles son propuestas y cuáles siguen abiertos. La equivalencia OPD↔OPL y las
reglas formales se comprueban cuando corresponda; no se exige una nueva
ratificación del operador para entregar un borrador, una explicación o una
realización ya autorizada.
...
**Criterio antes de refinar**: cada paso debe poder justificar:

1. **Qué pregunta del modelo se contesta con este OPD hijo?** — si no hay
   pregunta, proponer no refinar o marcar el propósito pendiente.
2. **Qué mecanismo de los cuatro corresponde y por qué?** — aplicar el mapa de
   decisión; consultar al operador sólo si dos alternativas legales cambian el
...
Tras cada paso de refinamiento, mantener bimodalidad y ejecutar la validación
pertinente. Un refinamiento sin contenido nuevo no cierra como refinamiento
canónico; puede conservarse como vista o propuesta si el régimen lo permite.
...
2. Compila y emite **con sello** via script bun ejecutado desde
   `<deep-opm-pro-root>/app/` (la API es TypeScript del repo):
...
```bash
cd <deep-opm-pro-root>/app && bun run verify:reproducible --proto <md> --golden <bundle.json>
...
Estado de **observabilidad**. Cuando el runtime y el comando se comprobaron,
produce un render con el renderer de opforja para inspeccionar regresiones de
layout o estructura. El artefacto acredita esa ejecución; no reemplaza la
validación de significado ni promete fidelidad fuera de la versión observada.

Precondición: existe un proto-modelo en OPL-ES estricto o un bundle y la sesión
dispone de un checkout compatible de `deep-opm-pro`. Resolver su ruta desde el
encargo o el workspace; no asumir un home. Si no está disponible, omitir esta
comprobación o usar `serializar-opd` con un renderer disponible, declarando la
diferencia.
...
1. **Renderizar headless.** Ejecutar (herramienta `Bash`):
   ...
   ```bash
   cd <deep-opm-pro-root>/app && bun run render:headless --proto <ruta-del-proto.md> --out <dir>
...
Regla de cierre del estado: si el encargo exige calidad visual o un render
opforja, inspeccionar la salida y registrar versión/comando observados. Para una
consulta conceptual o textual, el render no es un gate universal. La corrección
vive en el proto, no en la salida derivada.
...
- **Camino opforja: render headless de deep-opm-pro.** Si el checkout y el
  comando están disponibles y el destino requiere esa realización, usar
...
- **Render independiente: JointJS.** Cuando `deep-opm-pro` no está disponible o
  el encargo pide ese formato, generar el render con la librería
...
- bundle `deep-opm-pro.modelo.v0` cuando el destino lo requiera.
- (opcional) hook de render estatico via JointJS (ver `urn:dev:kb:jointjs-docs`).

Si el encargo autoriza escribir en la mesa y el carril fue comprobado, puede
usarse `mesa push` (§Puente directo). Para el camino manual, el bundle se importa por: **Abrir / importar
modelo** → gestor **Trabajo de modelado** → **Importar JSON** → pegar bundle.
La pestana resultante quedara marcada con chip de persistencia `Importado`.
...
5. **SD primero para refinamiento integrado; Bocetos para bottom-up**: el árbol formal nace en un SD, pero el operador PUEDE trazar Bocetos antes de decidirlo (metodologia §A1.5). Integrar exige destino y motivo de refinamiento; Graduar solo cambia el régimen y no integra ni completa el SD. En Modelo, la ausencia de cierre vuelve como pendiente exigible.
...
12. **Anti-barro**: no presentar como hecho un propósito, transformee, esencia,
    afiliación o motivo de refinamiento sin sustento. Corregir lo formalmente
    ilegal; marcar propuestas y bloquear sólo la parte que no puede cerrar.
...
13. **Corrección directa**: si el operador propone una primitiva mal aplicada,
    explicarlo con cita a la capa propietaria y proponer una forma legal.
14. **Aclaración focal**: preguntar sólo por la decisión material pendiente,
    sin repetir hechos ni imponer la plantilla como ceremonia.
15. **Decisión vs. incertidumbre**: conservar decisiones declaradas; dejar
    campos abiertos o proponer hipótesis revisables cuando eso permita avanzar.
    No atribuir a la persona lo que la skill propuso.
16. **Equivalencia OPD↔OPL**: comprobarla con la regla y el roundtrip aplicable.
    La aceptación humana del significado es un plano distinto y sólo se afirma
    con evidencia de esa validación.
...
20. **P3: normalización antes de compilación**: los verbos de dominio,
    morfología abierta y citas normativas se estandarizan en E2 por la skill.
    Exigir confirmación humana sólo para afirmar como aceptado un mapeo abierto;
    una propuesta rotulada puede seguir como tal. El compilador no aprende
    léxico abierto: verifica OPL-ES estricto y emite o rechaza con diagnóstico.
...
25. **Emisión M2**: cuando `deep-opm-pro` está disponible y su versión efectiva
    soporta el contrato, `serializar-bundle` usa el compilador de autoría (proto
    estricto → `compilarProto` → `emitirBundle` con `construirSello`). Registrar
    las comprobaciones realmente ejecutadas; sello, roundtrip, contención y
    golden-harness no se presumen por existir un archivo. El sello es
    procedencia estructural y guard antiaccidente, no firma. El bundle artesanal
    conserva sus límites y jamás simula o copia un sello.
...
28. **Dos ciclos, una sola identidad por nivel**: `esApunte` refleja el régimen documental y suspende **EXACTAMENTE** #12/#13; mantiene #1/#14/#15/#17 e integridad. La ruta visible es **Revisar para graduar → Graduar Apunte a Modelo** y su inversa **Reabrir en Taller**. Un Boceto es estado de componente: **Integrar como… / Devolver a Bocetos** no cambia `esApunte`. Integrar no gradúa; Graduar no integra. La integridad bloquea siempre; Bocetos y deuda formal permiten Graduar con pendientes, pero en Modelo bloquean o informan según R-CAN-BOCETO-1..4 y el gate aplicable. Ningún gesto certifica validación humana.
...
`deep-opm-pro` es el modelador OPM interactivo. Su ruta se obtiene del encargo o
del workspace efectivo; puede no estar disponible. Se subordina al **corpus
OPM/Forja SSOT ES**: la herramienta implementa y verifica, no redefine la
norma. El intercambio usa el documento JSON `deep-opm-pro.modelo.v0`.

### Capacidades de la app sobre las que se apoya esta skill

La tabla siguiente describe capacidades del contrato consultado. Antes de
depender de una, comprobarla en la versión y el entorno efectivos. Los rótulos
de ronda/corte indican procedencia y no prueban despliegue ni conducta:
...
| **OPL bimodal con eco en cada cambio** (ronda 20 / L2) | Si la capacidad está presente y las comprobaciones pasan, la app mantiene sincronía mecánica durante la edición; eso no valida el significado de dominio. |
...
| **Superficie «Piezas»: Calcar / Anclar desde bibliotecas** (2026-06-30, la PUERTA; retira el dock de biblioteca de ronda 20 / L3) | Cuando la versión lo soporta, **Calcar** trae una copia desacoplada y **Anclar** crea una referencia viva vigilada desde bibliotecas `esBiblioteca` de solo lectura. El bundle declara sólo las usadas. |
...
| **Ciclo documental reversible** (2026-07-27) | «Nuevo» crea un Apunte en Taller. **Revisar para graduar** separa integridad, Bocetos, cierre y validacion humana; integridad bloquea y el resto permite **Graduar con pendientes**. **Reabrir en Taller** devuelve el mismo Modelo de Trabajo a Apunte conservando ID, hechos y carpeta. El gestor **Trabajo de modelado** separa Taller, Modelos, Bibliotecas y Archivo; las carpetas organizan Taller y Modelos. |
| **Bocetos e integracion reversible** (2026-07-27; R-OPD-REF-20 / R-CAN-BOCETO-1..4) | Banda «Bocetos» al pie del arbol OPD; **Integrar como…** descomposicion/despliegue y **Devolver a Bocetos** preservan ID, contenido y subarbol. **Eliminar refinamiento** es destructivo y distinto. La voz visible evita «Adoptar»; `adoptarOpd` se conserva solo como identificador interno. |
...
| **Render headless** (H1, `bun run render:headless`) | Cuando el comando existe y pasa, produce PNG+SVG por OPD con el renderer de esa versión; alimenta `revisar-visual` y `serializar-opd` sin acreditar por sí solo significado ni fidelidad futura. |
...
| **Instancia configurada con login obligatorio** (auth v1, 2026-06-10) | Cuando la sesión identifica un despliegue vigente, el **carril de token del agente** autentica `mesa pull/push` contra ese backend; `push` sólo opera cuando el backend observado expone el commit atómico compatible (ver §Puente directo). El loop de compilación/render headless corresponde al checkout local descrito por el contrato. La versión desplegada se comprueba en el footer de «Ayuda › Atajos» (fecha de build + short SHA). |
...
con la instancia indicada por la configuración efectiva mediante el carril de
token del agente:

```bash
cd <deep-opm-pro-root>/app
...
Configuración: `OPFORJA_API_URL` y, cuando la sesión ya autoriza el carril, un
token Bearer resuelto por `OPFORJA_AGENT_TOKEN_FILE` o la configuración efectiva
del operador. No buscar ni copiar credenciales desde un home supuesto. El
carril se habilita del lado servidor con
...
Si el backend observado no expone el commit atomico de revisiones, `push` recibe
404/405/501 y aborta sin escribir: usar W6.0, nunca recurrir al endpoint antiguo
ni simular atomicidad con dos llamadas. La URL por defecto no prueba capacidad
desplegada: verificar el resultado real de `mesa pull/push` en la sesion y
declarar el fallback cuando corresponda.
...
`Testigo-Base`, el commit transaccional y la vitrina realizan el protocolo de
no-clobber. Verificar el resultado del comando; no inferirlo del bundle.
...
4a. **Camino manual (sin carril/token)**: indicar la apertura al usuario —
   `cd <deep-opm-pro-root>/app && bun run dev` (o la instancia que el encargo
   identifique y autorice),
   **Abrir / importar modelo** → gestor
   **Trabajo de modelado** → **Importar JSON** → pegar el bundle. Si el bundle
   proviene del **compilador de autoria** (`emitirBundle`), lo que se pega es
   el campo `.json` del
...
Si el entregable exige edición interactiva en `deep-opm-pro`, producir el bundle
correspondiente. Un modelo conceptual o dictamen no adquiere esa obligación por
la mera disponibilidad de la UI. El render JointJS sirve a entregables visuales
que no usarán el entorno interactivo.
...
- `referencias/catalogo-de-barro.md` — ambigüedades y anti-patrones de modelado,
  con ejemplos y plantillas para las aclaraciones que sí resulten materiales.
...
Las referencias son **resúmenes operativos curados**, no SSOT. La SSOT primaria
de esta skill es el corpus OPM/Forja SSOT ES:
`urn:fxsl:kb:reglas-opm-estrictas-es`, `urn:fxsl:kb:spec-forja-opd-es`,
`urn:fxsl:kb:spec-forja-opl-es`, `urn:fxsl:kb:metodologia-forja-opm-es` y
`urn:fxsl:kb:opm-categorial-es`. Las capas base se usan como fuentes delegadas.
La SSOT del shape JSON del bundle es el código de la versión efectiva del
modelador (`<deep-opm-pro-root>/app/src/serializacion/json.ts` y
`app/src/modelo/tipos/`); si el código tensiona con la semántica OPM, manda el
corpus Forja y se corrige la herramienta.
