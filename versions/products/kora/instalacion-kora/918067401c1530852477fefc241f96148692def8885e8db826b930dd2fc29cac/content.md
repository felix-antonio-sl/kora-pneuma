...
sobre ellas. Cuando hay consumidores compartidos, exclusiones, cambio de nombre
o propiedad incierta, prepara primero `install TARGET URN --dry-run` y revisa
efectos por archivo y propietario; `--plan ARCHIVO` permite aplicar el plan
contrastando sus precondiciones bajo lock. Aísla un conflicto protegido y sigue
con selecciones independientes. El comando comprueba dependencias, realiza archivos nativos y
...
Comprueba el resultado de la operación y usa `status --compare-source` para reconocer fuente cambiada, dependencias pendientes y desvíos o
