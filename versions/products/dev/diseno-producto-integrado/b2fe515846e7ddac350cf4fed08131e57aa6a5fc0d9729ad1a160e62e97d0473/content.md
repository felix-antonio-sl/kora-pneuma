...
1. Conservar fuentes y pruebas; usar `EVIDENCE_LEDGER` numerado si ayuda a revisar. La
   numeración `[E#]` es obligatoria aunque no se use ledger separado: cada entrada
   vive en el ledger cuando existe, o numerada inline en la sección de evidencia
   en caso contrario. Admite
...
2. Cada afirmación `verificado` debe citar un `[E#]` definido en esta ejecución
   y su insumo, ruta o comando
   con resultado; esa entrada es su fuente observable de esta ejecución. Un `[E#]`
   sin entrada correspondiente no acredita nada.
