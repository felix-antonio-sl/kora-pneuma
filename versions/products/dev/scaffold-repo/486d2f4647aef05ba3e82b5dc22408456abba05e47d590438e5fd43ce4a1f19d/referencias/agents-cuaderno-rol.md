# AGENTS.md

## Misión

Cuaderno operativo de **{{rol}}** en {{contexto}}: {{resultado que decide, produce o entrega}}.

## Entrada y autoridad

- Usa `README.md` como entrada humana y las fuentes oficiales del dominio como autoridad.
- Si existe `HANDOFF.md`, léelo solo al retomar trabajo material inconcluso y
  revalídalo contra sistemas y fuentes actuales.
- {{sistemas vecinos, autoridades institucionales y límites de decisión}}

## Principio curatorial

- Entra lo que el rol produce, firma, presenta o usa; el insumo estable se referencia,
  no se copia.
- Este cuaderno no es un repositorio de código: no agregues dependencias ni servicios.
- {{mapa de directorios, tipos documentales y convenciones propias}}

## Verificación

{{forma concreta de comprobar el resultado que entrega este cuaderno}}

- Verifica fechas, versiones, atribución y estado vivo en la fuente correspondiente.
- Separa evidencia técnica de autorización humana, clínica, institucional o productiva.

## Seguridad y entrega

- No vuelques secretos, PII/PHI ni datos sensibles; usa síntesis desidentificada cuando
  el encargo lo autorice.
- No envíes, publiques ni firmes fuera de la autoridad solicitada.
