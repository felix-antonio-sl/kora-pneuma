...
| Contrato de conocimiento en la transmutación | **Descubierta y cerrada 2026-06-14.** `emitir()` proyectaba `name`/`description`/`herramientas` + cuerpo + sello pero ignoraba `conocimiento`/`componible`: el output instalado no portaba los URN del corpus permitido ni cómo alcanzarlos en el repo central — el cuerpo de algunos (p. ej. `urgenciologo`) prometía "los URN declarados" que el output no llevaba. El encuadre inicial —*elegir entre modelos de despliegue de rutas*— era falso: el path ya es **función pura del URN**; `lugar-coincide` blinda la biyección `urn:{ns}:kb:{id}` ⟷ `artefactos/conocimiento/{ns}/{id}.md` en cada `velar`, así que no hay ruta que decidir ni hornear. El sello porta ahora un `contrato-conocimiento:` con el **ancla** al repo central (`~/kora-pneuma` / `$KORA_RAIZ`), la **regla de derivación** declarada una vez y la **lista de URN** (`conocimiento` + `componible`); cero paths materializados, el URN como única autoridad. Un agente con solo `Read` resuelve `{ancla}/{path}` por sustitución, sin Bash; determinista, re-emitible y rancio como el `hash-fuente`. La bestia horneaba `KB Routes` relativas a su estructura, frágiles bajo CWD arbitrario; pneuma **cita la función en vez de recalcular el resultado**. Cerrada la deuda, `--aplicar` de los agénticos deja de degradar el acceso al corpus respecto a la bestia. |
