...
| Contrato de conocimiento en la transmutación | **Descubierta 2026-06-14.** `emitir()` (kora.py) proyecta `name`/`description`/`herramientas` + cuerpo + sello, pero **ignora el campo `conocimiento`**: el output a claude-code/codex/opencode no lleva las rutas ni los URNs del corpus permitido. La fuente ya es coherente (los 24 agénticos declaran 242 refs de conocimiento y **todas resuelven** en el censo), pero el cuerpo de algunos (p. ej. `urgenciologo`) promete "los URNs declarados en el frontmatter", y el frontmatter del output no los porta → promesa rota solo en runtime. La bestia sí emitía un `Knowledge Contract`/`KB Routes`, pero con rutas relativas a su estructura (subdirs que pneuma aplanó), frágiles bajo CWD arbitrario. Cerrarla exige decidir el modelo de despliegue de conocimiento (URN+`kora.py nombre` / URN→ruta absoluta pneuma / empacar fibra) y proyectarlo en `emitir`. Mientras tanto, `--aplicar` de los agénticos al runtime queda gateado: degradaría el acceso al corpus respecto a la versión bestia instalada. |
